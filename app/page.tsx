  ...
  return (
    <div className="min-h-screen bg-[#08080E] text-[#F0EDE6] selection:bg-[#C9A84C] selection:text-black font-cairo overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#08080E]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="text-2xl font-bold tracking-tighter uppercase hidden sm:block text-white">
              Degitale
            </span>
          </div>

          <div className="hidden md:flex items-center gap-10 text-sm font-medium text-gray-400">
            <Link href="/shop"       className="hover:text-[#C9A84C] transition-all whitespace-nowrap">المتجر</Link>
            <Link href="/categories" className="hover:text-[#C9A84C] transition-all whitespace-nowrap">التصنيفات</Link>
            <Link href="/sell"       className="hover:text-[#C9A84C] transition-all whitespace-nowrap">ابدأ البيع</Link>
          </div>

          <Link
            href="/login"
            className="bg-white/5 border border-white/10 px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-[#C9A84C] hover:text-black transition-all shrink-0 text-white whitespace-nowrap"
          >
            دخول
          </Link>
        </div>
      ...
      <section className="relative pt-52 pb-32 overflow-hidden flex flex-col items-center">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1200px] h-[700px] bg-[#C9A84C]/10 blur-[130px] rounded-full -z-10 pointer-events-none" />
        
        <div className="max-w-5xl mx-auto px-6 flex flex-col items-center text-center relative z-10 w-full font-serif">
          <div className="inline-block px-5 py-2 mb-8 rounded-full border border-[#C9A84C]/20 bg-[#C9A84C]/5 text-[#C9A84C] text-[10px] font-black tracking-[0.3em] uppercase whitespace-nowrap">
            مستقبل التجارة الرقمية الحصرية
          </div>
          
          <h1 className="text-5xl md:text-8xl font-bold mb-10 leading-[1.3] text-white">
            امتلك أفضل
            <br />
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center w-full max-w-md">
            <Link
              href="/shop"
              className="w-full sm:w-auto bg-[#C9A84C] text-[#08080E] px-14 py-4 rounded-full font-black text-lg hover:scale-105 transition-all shadow-[0_0_40px_rgba(201,168,76,0.25)] text-center whitespace-nowrap"
            >
              تصفح المنتجات
            </Link>
            <Link
              href="/sell"
              className="w-full sm:w-auto px-14 py-4 rounded-full border border-white/10 font-bold text-lg hover:bg-white/5 transition-all text-white text-center whitespace-nowrap"
            >
              ابدأ البيع
            </Link>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 w-full">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6 w-full">
          ...
          <Link
            href="/shop"
            className="text-[#C9A84C] font-bold border-b-2 border-[#C9A84C]/20 pb-1 hover:border-[#C9A84C] transition-all text-sm uppercase tracking-widest shrink-0 whitespace-nowrap"
          >
            تصفح المتجر بالكامل ←
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 w-full">
          {featuredProducts.length > 0 ? (
            ...
            [1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="aspect-[4/5] rounded-[2.5rem] bg-white/5 border border-white/5 animate-pulse"
              />
            ))
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-12 w-full">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 order-2 md:order-1">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-white/10 rounded flex items-center justify-center text-[10px] font-bold text-[#C9A84C]">
                D
              </div>
              <span className="font-bold text-white tracking-widest uppercase text-sm">Degitale</span>
            </div>
            <span className="text-xs text-gray-600 font-medium">
              © 2026 جميع الحقوق محفوظة لمنصة ديجيتال
            </span>
          </div>
